...
#include "calib_huawei_camera.hpp"
...
void roughCalib(Calibration& calibra, double search_resolution, int max_iter)
{
  ...
  for(int n = 0; n < 2; n++)
    ...
    {
      Vector3d euler_angle = calibra.camera_.ext_R.eulerAngles(2, 1, 0); // Z-Y-X
...
{
  ros::init(argc, argv, "calib_huawei_camera");
  ros::NodeHandle nh;

  int dis_thr_low_bound, data_num;
  bool use_ada_voxel, use_rough_calib;
  ...
  {
    Calibration calib(CamConfigPath, "/home/sam/catkin_ws/src/mlcc/config/fr/huawei" + to_string(i) + ".yaml", use_ada_voxel);
    calibs.push_back(calib);
  }
  
  if(use_rough_calib) roughCalib(calibs[0], DEG2RAD(0.1), 30);
  ...
  for(int dis_threshold = 30; dis_threshold > dis_thr_low_bound; dis_threshold -= 1)
  {
    ...
    {
      Eigen::Vector3d euler_angle = calibs[0].camera_.ext_R.eulerAngles(2, 1, 0); // 2 is z, 0 is x
      Eigen::Vector3d transation = calibs[0].camera_.ext_t;
      ...
      Eigen::Matrix3d R;
      Eigen::Vector3d T;
      R = calibs[0].camera_.ext_R;
      T = calibs[0].camera_.ext_t;
      
      inner << calibs[0].camera_.fx_, calibs[0].camera_.s_, calibs[0].camera_.cx_,
               0, calibs[0].camera_.fy_, calibs[0].camera_.cy_,
               0, 0, 1;
      #ifdef FISHEYE
      distor << calibs[0].camera_.k1_, calibs[0].camera_.k2_, calibs[0].camera_.k3_, calibs[0].camera_.k4_;
      #else
      ...
      for(int i = 0; i < data_num; i++)
      {
        ...
        calibs[i].buildVPnp(calibs[i].camera_, calib_params, dis_threshold, true,
                            calibs[i].camera_.rgb_edge_cloud_, calibs[i].lidar_edge_cloud_, vpnp_list);
      ...
      for(int i = 0; i < data_num; i++)
        calibs[i].camera_.update_Rt(m_q.toRotationMatrix(), m_t);
  ...
  Eigen::Vector3d euler_angle = calibs[0].camera_.ext_R.eulerAngles(2, 1, 0);
  Eigen::Vector3d transation = calibs[0].camera_.ext_t;
  Vector6d calib_params;
  calib_params << euler_angle(0), euler_angle(1), euler_angle(2),
                  transation(0), transation(1), transation(2);
  ...
  calib_params << euler_angle(0), euler_angle(1), euler_angle(2),
                  transation(0), transation(1), transation(2);
  calibs[0].colorCloud(calib_params, 1, calibs[0].camera_, calibs[0].camera_.rgb_img_, calibs[0].lidar_cloud_);
